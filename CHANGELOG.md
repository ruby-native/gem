...
- `ruby_native screenshots` no longer crashes with `TypeError: no implicit conversion of Symbol into Integer` when `config/ruby_native.yml` has a `screenshots:` (or `ruby_native:`) key that is not a hash. The CLI now treats a malformed or unexpectedly-shaped section as "not configured" and falls through to the interactive setup.
